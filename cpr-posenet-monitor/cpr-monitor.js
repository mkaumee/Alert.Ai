...
let lastBeepTime = 0; // Track when last beep occurred
...
async function preloadDetector() {
    ...
    updateModelLoadingStatus('Loading AI Model', 'Downloading TensorFlow.js model (~12MB)');
    ...
    try {
        ...
        updateModelLoadingStatus('Initializing Model', 'Setting up pose detection...');
        ...
        updateModelLoadingStatus('Model Ready', 'CPR monitoring is ready to start');
        ...
        updateModelLoadingStatus('Loading Failed', error.message || 'Please refresh and try again');
        
        throw error;
    }
}

// Function to update model loading status
function updateModelLoadingStatus(title, message) {
    const loadingIndicator = document.getElementById('model-loading-indicator');
    if (loadingIndicator && modelLoading) {
        const spinner = loadingIndicator.querySelector('div[style*="animation"]');
        const titleEl = loadingIndicator.querySelector('div[style*="font-weight: bold"]');
        const messageEl = titleEl ? titleEl.nextElementSibling : null;
        
        if (titleEl) titleEl.textContent = title;
        if (messageEl) {
            messageEl.innerHTML = `${message}<br><span style="color: #ff4444;">Please wait...</span>`;
        }
    }
}
...
async function startCPRDetection() {
    ...
    try {
        ...
        setTimeout(() => {
            speak("CPR Monitor active. Follow the beep rhythm for chest compressions.", true);
        }, 2000);  // Increased delay to ensure camera is ready
        
        // Start periodic coaching even without pose detection
        setTimeout(() => {
            startPeriodicCoaching();
        }, 10000); // Start coaching after 10 seconds
...
function stopCPRDetection() {
    ...
    // Stop periodic coaching
    stopPeriodicCoaching();
...
function updateCPRDisplays(cpm, count) {
    ...
    // Provide coaching feedback - reduced threshold for faster feedback
    if (count >= 2) {  // Changed from 5 to 2 for faster coaching
        provideCPRCoaching(cpm);
    } else if (count === 1) {
        // Give encouragement after first compression
        speak("Good! Keep going. Follow the beep.", true);
    }
}
...
function playBeep() {
    ...
    // Increased metronome volume from 0.3 to 0.6 for better audibility
    ...
    // Track when beep occurred for speech timing coordination
...
function speak(text, priority = false) {
    ...
    try {
        ...
        // Smart timing: avoid speaking right after a beep to let metronome be heard
        const timeSinceLastBeep = Date.now() - lastBeepTime;
        const beepInterval = 60000 / TARGET_CPM; // Time between beeps
        
        // If we just had a beep (within 200ms), wait a bit before speaking
        if (metronomeActive && timeSinceLastBeep < 200) {
            setTimeout(() => speak(text, priority), 300);
            return;
        }
        ...
        utterance.pitch = 1.0;
        utterance.volume = 0.5; // Reduced from 0.8 to 0.5 to balance with metronome
        utterance.lang = 'en-US';
        
        // Add event listeners for debugging
        utterance.onstart = function() {
            console.log('[CPR Coach] Speech started:', text);
        };
        
        utterance.onend = function() {
            console.log('[CPR Coach] Speech ended');
        };
        
        utterance.onerror = function(event) {
            console.error('[CPR Coach] Speech error:', event.error);
        };
...
// Periodic coaching system (fallback when pose detection isn't working)
let periodicCoachingInterval = null;
let coachingMessages = [
    "Keep going. Follow the beep.",
    "Push hard and fast. Good rhythm.",
    "You're doing great. Stay focused.",
    "Keep your arms straight.",
    "Continue compressions. Follow the beep.",
    "Perfect. Maintain this pace."
];
let currentCoachingIndex = 0;

function startPeriodicCoaching() {
    if (periodicCoachingInterval) return;
    
    periodicCoachingInterval = setInterval(() => {
        if (cprActive) {
            const message = coachingMessages[currentCoachingIndex];
            speak(message, false); // Non-priority to allow timing coordination
            currentCoachingIndex = (currentCoachingIndex + 1) % coachingMessages.length;
        }
    }, 20000); // Increased from 15 to 20 seconds to reduce frequency
    
    console.log('[CPR Monitor] Periodic coaching started');
}

function stopPeriodicCoaching() {
    if (periodicCoachingInterval) {
        clearInterval(periodicCoachingInterval);
        periodicCoachingInterval = null;
        console.log('[CPR Monitor] Periodic coaching stopped');
    }
}

// Test speech function for user interaction
function testSpeech() {
    speak("Voice coaching is working. You'll receive guidance during CPR.", true);
}

function provideCPRCoaching(cpm) {
    const now = Date.now();
    
    // Provide coaching feedback every 4 seconds (increased from 3 to reduce frequency)
    if (now - lastFeedbackTime < 4000) return;
    ...
    if (cpm === 0) {
        voiceMessage = "Begin compressions. Follow the beep.";
        displayMessage = "Start Compressions";
    } else if (cpm < 90) {
        voiceMessage = `Too slow. Speed up to ${TARGET_CPM}.`;
        displayMessage = `Too Slow: ${cpm} CPM`;
    } else if (cpm >= 90 && cpm < 100) {
        voiceMessage = `Speed up slightly. Target ${TARGET_CPM}.`;
        displayMessage = `Speed Up: ${cpm} CPM`;
    } else if (cpm >= 100 && cpm <= 120) {
        voiceMessage = `Perfect rhythm! Keep going.`;
        displayMessage = `Perfect: ${cpm} CPM`;
    } else if (cpm > 120 && cpm <= 140) {
        voiceMessage = `Slow down a bit. Follow the beep.`;
        displayMessage = `Slow Down: ${cpm} CPM`;
    } else {
        voiceMessage = `Too fast. Follow the beep rhythm.`;
        displayMessage = `Too Fast: ${cpm} CPM`;
    }
    
    // Speak the coaching message (non-priority for timing coordination)
